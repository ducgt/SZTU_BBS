<template>
  <view>
    <view class="model">
      <navigator hover-class="none" v-for="(item,index) in model" :key="index" class="model-item"
                 :style="{backgroundColor: item.color}"
                 :url="'../model_detail/model_detail?model_id='+item.id">
    ...
    <view class="hot-topic" v-for="(item_model,index_model) in model" :key="index_model">
      <navigator hover-class="none" class="header" :style="{borderLeftColor: item_model.color}"
                 :url="'../model_detail/model_detail?model_id='+item_model.id">
      ...
      <navigator hover-class="none" :url="'../topic_detail/topic_detail?topic_id='+item.id"
                 :class="{top_border:index_topic!==0}"
                 class="hot-topic-item" v-for="(item, index_topic) in item_model.topic" :key="index_topic">
    ...
    <view v-if="is_show_personal" class="personal_">
      <view class="personal">
        ...
        <navigator hover-class="none" style="font-size:14px" url="../school_confirm/school_confirm"
  ...
  export default {
    data() {
      return {
        inputShowed: false,
        inputVal: "",
        model: uni.getStorageSync('index_data_2') || [],
        floorstatus: false,
    ...
    onLoad: function () {
      ...
      uni.request({
        url: getApp().globalData.host+'article/get_index2_data/',
        data: {
    ...
    onHide() {
      wx.setStorageSync('index_data_2', this.model)
    }
  }
</script>
